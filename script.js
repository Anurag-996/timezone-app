...
async function displayData(ans, key) {
  ...
  document.querySelector(`#name${key}`).textContent = name;
  document.querySelector(`#lat${key}`).textContent = latitude;
  document.querySelector(`#std${key}`).textContent = offsetStd;
  document.querySelector(`#std-seconds${key}`).textContent = offsetStd_sec;
  document.querySelector(`#dst${key}`).textContent = offsetDst;
  document.querySelector(`#dst-seconds${key}`).textContent = offsetDst_sec;
  document.querySelector(`#country${key}`).textContent = country;
  document.querySelector(`#postcode${key}`).textContent = postcode;
  document.querySelector(`#city${key}`).textContent = city;
  document.querySelector(`#long${key}`).textContent = longitude;
  console.log(ans);
}
